...
const AppInner: React.FC = () => {
  ...
  if (loadError) {
    return (
      <div className="flex flex-col h-screen w-screen items-center justify-center bg-yuri-50 p-6 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-yuri-200 flex flex-col items-center w-80">
          <h2 className="text-xl font-bold text-yuri-900 mb-2 text-center">데이터를 불러오지 못했습니다.</h2>
          <p className="text-sm text-yuri-500 mb-6 text-center">인터넷 연결을 확인한 후 다시 시도해주세요.</p>
          <button onClick={() => window.location.reload()} className="w-full bg-red-400 hover:bg-red-500 text-white font-bold py-3.5 rounded-xl transition-colors">
            다시 시도
          </button>
        </div>
      </div>
